...
- Between 768px and 1100px, where the panel sits above a short map, the heading keeps only the service counts and the reading time and leaves the tally and names to the panel; a portrait tablet has room above the map, so its heading keeps them too. On a phone 700px tall or less the heading still counts every verdict but leaves the names to the panel; a `down` or `degraded` body still says so on its own label.
